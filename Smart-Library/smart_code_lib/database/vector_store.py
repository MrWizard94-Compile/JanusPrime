...
class VectorMemoryStore:
    ...
    def query_context(self, query: str, limit: int = 3) -> str:
        ...
        slices = self.query_context_slices(query, limit=limit)
        return "\n\n".join(slices)

    def query_context_slices(
        self,
        query: str,
        *,
        limit: int = 3,
        ...
        """Return bounded context slices without LLM synthesis (token-efficient)."""
        ...
        for result in results:
            ...
            slice_text = f"[{category}]: {result.page_content}"
            if total_chars + len(slice_text) > max_chars:
                remaining = max_chars - total_chars
                if remaining > 0:
                    slices.append(slice_text[:remaining])
                break
            slices.append(slice_text)
            total_chars += len(slice_text)

        return slices

    def content_hashes_for_category(self, category: str, limit: int = 1000) -> list[str]:
        """Return SHA-256 hashes of document content for a metadata category."""
        documents = self.list_documents(limit=limit)
        hashes: list[str] = []
