...
class SelfHealingSandbox:
    ...
    def heal_and_verify(self, broken_code: str, max_attempts: int = 3) -> Dict[str, Any]:
        ...
        last_fix_explanation: str | None = None
        ...
        for attempt in range(max_attempts):
            ...
            if result["success"]:
                if attempt > 0 and last_fix_explanation and last_error_traceback:
                    self.db.insert_reference(
                        content=(
                            f"Fixed error: {last_error_traceback}. "
                            f"Fix: {last_fix_explanation}"
                        ),
                        category="Self-Healing Patch",
                    )
                status = "Healed" if attempt > 0 else "Success"
                return {
                    "status": status,
                    "code": current_code,
                    "attempts": attempt + 1,
            ...
            last_error_traceback = result.get("error_traceback")
            last_fix_explanation = parsed_fix.get(
