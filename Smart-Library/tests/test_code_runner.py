...
def test_heal_and_verify_success_first_attempt(mock_check, mock_llm):
    """First-attempt success returns Success and does not seed memory."""
    mock_db = MagicMock()
    sandbox = SelfHealingSandbox(vector_db=mock_db)
    ...
    mock_db.insert_reference.assert_not_called()


@patch("smart_code_lib.sandbox.code_runner.get_chat_llm")
@patch("smart_code_lib.sandbox.code_runner.check_ollama_available", return_value=(True, ""))
def test_heal_and_verify_repairs_then_succeeds(mock_check, mock_llm):
    """Verified retry stores the patch only after the fixed code executes successfully."""
    mock_db = MagicMock()
    sandbox = SelfHealingSandbox(vector_db=mock_db)
    ...
    mock_db.insert_reference.assert_called_once_with(
        content="Fixed error: NameError. Fix: defined var",
...
@patch("smart_code_lib.sandbox.code_runner.get_chat_llm")
@patch("smart_code_lib.sandbox.code_runner.check_ollama_available", return_value=(True, ""))
def test_heal_and_verify_does_not_store_patch_before_verification(mock_check, mock_llm):
    """Intermediate LLM fixes are not written until a retry succeeds (attempt > 0)."""
    mock_db = MagicMock()
    sandbox = SelfHealingSandbox(vector_db=mock_db)
    mock_llm_instance = MagicMock()
    mock_llm_instance.invoke.return_value = MagicMock(
        content=json.dumps({"fixed_code": "still broken", "explanation": "tried fix"})
    )
    sandbox.llm = mock_llm_instance
...
def test_heal_and_verify_exhausts_max_attempts(mock_check, mock_llm):
    """Healing fails after max_attempts without seeding unverified patches."""
    mock_db = MagicMock()
    sandbox = SelfHealingSandbox(vector_db=mock_db)
    mock_llm_instance = MagicMock()
    mock_llm_instance.invoke.return_value = MagicMock(
...
@patch("smart_code_lib.sandbox.code_runner.get_chat_llm")
@patch("smart_code_lib.sandbox.code_runner.check_ollama_available", return_value=(True, ""))
def test_heal_and_verify_stores_patch_only_after_verified_retry(mock_check, mock_llm):
    """Patch write-back happens once, after the healed code passes execution."""
    mock_db = MagicMock()
    sandbox = SelfHealingSandbox(vector_db=mock_db)
    mock_llm_instance = MagicMock()
    mock_llm_instance.invoke.side_effect = [
        MagicMock(
            content=json.dumps(
                {"fixed_code": "print('v1')", "explanation": "first fix"}
            )
        ),
        MagicMock(
            content=json.dumps(
                {"fixed_code": "print('v2')", "explanation": "second fix"}
            )
        ),
    ]
    sandbox.llm = mock_llm_instance
    ...
    outcomes = [
        {"success": False, "stdout": "", "error_traceback": "Error v0"},
        {"success": False, "stdout": "", "error_traceback": "Error v1"},
        {"success": True, "stdout": "v2\n", "error_traceback": None},
    ]

    with patch.object(sandbox, "safely_execute_python", side_effect=outcomes):
        report = sandbox.heal_and_verify("broken", max_attempts=3)

    assert report["status"] == "Healed"
    assert report["attempts"] == 3
    mock_db.insert_reference.assert_called_once_with(
        content="Fixed error: Error v1. Fix: second fix",
        category="Self-Healing Patch",
    )