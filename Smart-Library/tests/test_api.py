...
def test_doctrine_status_endpoint(client, mock_services):
    """Doctrine status reports whether content hash exists in Operational Doctrine."""
    _, mock_db, _, _ = mock_services
    mock_db.content_hashes_for_category.return_value = ["abc123"]

    response = client.get("/doctrine/status?content_hash=abc123")
    ...
    mock_db.content_hashes_for_category.assert_called_once_with("Operational Doctrine")
