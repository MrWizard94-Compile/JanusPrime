...
def test_content_hashes_for_category_filters_metadata():
    """Category hash lookup returns SHA-256 digests for matching documents only."""
    collection = MagicMock()
    collection.get.return_value = {
        "ids": ["id-1", "id-2"],
        "documents": ["soul content", "other content"],
        "metadatas": [
    ...
    hashes = store.content_hashes_for_category("Operational Doctrine")

    assert hashes == [hashlib.sha256("soul content".encode("utf-8")).hexdigest()]