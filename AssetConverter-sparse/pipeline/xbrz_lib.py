...
try:
    ...
    _HAS_NATIVE_XBRZ = True
except ImportError:
    _HAS_NATIVE_XBRZ = False
    xbrz_path = None
...
def scale_pillow(img: 'PIL.Image.Image', factor) -> 'PIL.Image.Image':
	...
	if not _HAS_NATIVE_XBRZ:
		from pipeline.pixel_scaler import upscale_rgba as hq2x_fallback
		if factor != 2:
			raise ValueError("hq2x fallback only supports 2x")
		return hq2x_fallback(img.convert("RGBA"))
